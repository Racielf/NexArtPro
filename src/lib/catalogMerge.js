...
/**
 * Ensure all seed Services exist in the live NexArt Service entity.
...
export async function ensureServicesComplete(liveServices) {
  ...
  for (let i = 0; i < toInsert.length; i += 50) {
    await nexartClient.entities.Service.bulkCreate(toInsert.slice(i, i + 50));
  }
...
export async function ensurePriceBookComplete(livePB, liveServices) {
  ...
  for (let i = 0; i < toInsert.length; i += 50) {
    await nexartClient.entities.PriceBookEntry.bulkCreate(toInsert.slice(i, i + 50));
  }
