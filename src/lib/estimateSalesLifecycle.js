...
export async function generatePublicShareToken(estimate) {
  ...
  await nexartClient.entities.Estimate.update(estimateId, {
...
export async function markEstimateSent(estimateId, { documentConfig, estimate, currentUser } = {}) {
  ...
  try {
    await nexartClient.entities.EstimateSnapshot.create({
  ...
  await nexartClient.entities.Estimate.update(estimateId, payload);
  return payload;
}
