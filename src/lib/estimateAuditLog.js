...
export async function logChange({
  ...
  try {
    ...
    await nexartClient.asServiceRole.entities.EstimateVersionHistory.create({
      estimate_id,
      estimate_number,
...
export async function fetchAuditLog(estimate_id) {
  try {
    const entries = await nexartClient.asServiceRole.entities.EstimateVersionHistory.filter({
      estimate_id,
      archived_reason: 'audit_log',
    });
