...
export async function actionOMW(appt) {
  ...
  // Update appointment
  await nexartClient.entities.Appointment.update(appt.id, {
    status: 'on_the_way',
    omw_started_at: now,
  });

  // Create travel log
  const log = await nexartClient.entities.TimeTrackingLog.create({
    appointment_id: appt.id,
...
export async function actionArrived(appt) {
  ...
  const activeLogs = await nexartClient.entities.TimeTrackingLog.filter({
    appointment_id: appt.id,
    tracking_type: 'travel',
    status: 'active',
  });
  ...
  for (const log of activeLogs) {
    ...
    await nexartClient.entities.TimeTrackingLog.update(log.id, {
      end_time: now,
  ...
  // Start on-site log
  await nexartClient.entities.TimeTrackingLog.create({
    appointment_id: appt.id,
    customer_id: appt.customer_id || '',
    customer_name: appt.customer_display_name,
    worker_name: appt.assigned_worker_name || '',
