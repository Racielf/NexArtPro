...
async function issueSigningAccessForPackage({ pkg }) {
  ...
  const ordered = [];
  const existing = await nexartClient.entities.SigningParticipant.filter({ signing_package_id: pkg.id }).catch(() => []);
  ...
  if (activeParticipant?.id) {
    ...
    await nexartClient.entities.SigningParticipant.update(activeParticipant.id, {
      ...fields,
  ...
  const fields = await buildTokenFields(token, now);
  await nexartClient.entities.SigningPackage.update(pkg.id, fields).catch(() => {});
...
async function syncSigningParticipants({ pkg, estimate, currentUser }) {
  ...
  const existingParticipants = await nexartClient.entities.SigningParticipant
  ...
  for (const participant of desiredParticipants) {
    ...
    if (existing?.id) {
      ...
      const nextStatus = CLOSED_PARTICIPANT_STATUSES.has(existing.status)
        ? existing.status
        : participant.status;
      await nexartClient.entities.SigningParticipant.update(existing.id, {
        ...patch,
        ...legacyFields,
        status: nextStatus,
      }).catch(() => {});
    ...
    const created = await nexartClient.entities.SigningParticipant.create({
  ...
  for (const participant of staleParticipants) {
    ...
    await nexartClient.entities.SigningParticipant.update(participant.id, {
      status: 'voided',
      token: '',
      token_hash: '',
      token_last_four: '',
  ...
  const refreshedParticipants = await nexartClient.entities.SigningParticipant
    .filter({ signing_package_id: pkg.id })
    .catch(() => []);
  ...
  if (!activeParticipant && nextPendingParticipant?.id) {
    await nexartClient.entities.SigningParticipant.update(nextPendingParticipant.id, {
      status: 'active',
      sent_at: nextPendingParticipant.sent_at || pkg.sent_at || new Date().toISOString(),
    }).catch(() => {});
  }

  const normalizedParticipants = (await nexartClient.entities.SigningParticipant
    .filter({ signing_package_id: pkg.id })
    .catch(() => []))
    .sort((a, b) => (a.signing_order || 1) - (b.signing_order || 1));
  ...
  if (newlyCreated.length > 0) {
    await nexartClient.entities.SigningEvent.create({
      signing_package_id: pkg.id,
      document_type: pkg.document_type,
      document_id: pkg.document_id,
  ...
  if (activeAfterSync?.id) {
    const alreadyActivated = await nexartClient.entities.SigningEvent
...
export async function createSigningPackageForEstimate({ estimate, pdfUrl = '', pdfName = '', pdfHash = '', currentUser = null }) {
  ...
  const existing = await nexartClient.entities.SigningPackage.filter({
  ...
  if (reusable?.id) {
    const normalizedReusable = await backfillLegacyToken(nexartClient.entities.SigningPackage, reusable);
    const patch = {
      token: '',
    ...
    if (Object.keys(patch).length > 0) {
      await nexartClient.entities.SigningPackage.update(normalizedReusable.id, patch).catch(() => {});
    }
  ...
  const pkg = await nexartClient.entities.SigningPackage.create({
