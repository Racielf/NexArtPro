 ...
 */
async function computeLaborFromEntries(timeEntries, base44) {
  ...
  if (workerIds.length) {
    ...
    const results = await Promise.all(
      workerIds.map(id => base44.entities.Worker.filter({ id }))
    );
...
export async function getJobFinancials(workOrderId, base44) {
  ...
  const [woList, expenses, invoices, timeEntries] = await Promise.all([
    ...
    base44.entities.WorkOrderTimeEntry.filter({ work_order_id: workOrderId }),
  ]);
  ...
  const entryData = timeEntries?.length
    ? await computeLaborFromEntries(timeEntries, base44)
    : null;
