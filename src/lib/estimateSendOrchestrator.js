...
export async function executeSend({ estimate, recipientEmail, subject, message, currentTemplate, currentOptions, includedAttachmentIds = [], appConfig }) {
  ...
  try { currentUser = await nexartClient.auth.me().catch(() => null); } catch {}
  ...
  let emailRes;
  try {
    emailRes = await nexartClient.functions.invoke('sendEstimateEmail', {
      to: recipientEmail,
      subject,
      message,
  ...
  try {
    ...
    const snapshots = await nexartClient.entities.EstimateSnapshot.filter({ estimate_id: estimate.id }, '-created_date', 1).catch(() => []);
  ...
  if (pdfUrl && snapshotId) {
    try {
      await nexartClient.entities.EstimateSnapshot.update(snapshotId, { pdf_file_url: pdfUrl, pdf_file_name: pdfFilename || `estimate-${estimate.estimate_number}.pdf`, pdf_file_hash: pdfHash || '', hash_algorithm: pdfHash ? 'SHA-256' : '' });
    } catch {}
  }
