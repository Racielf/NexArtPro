 ...
 *
 * Uses NexArt client entities as the source of truth:
...
import { invalidateNexArtServiceCache } from '@/components/shared/services/serviceSearchNexArt';
...
export async function loadServices() {
  let records = await nexartClient.entities.Service.list('-created_date', 500);
  if (records.length === 0) {
    ...
    for (let i = 0; i < seedData.length; i += 50) {
      await nexartClient.entities.Service.bulkCreate(seedData.slice(i, i + 50));
    }
    records = await nexartClient.entities.Service.list('-created_date', 500);
  } else {
...
export async function createService(data) {
  const result = await nexartClient.entities.Service.create(data);
  await invalidateAllCaches();
  return result;
}

export async function updateService(id, data) {
  const result = await nexartClient.entities.Service.update(id, data);
  await invalidateAllCaches();
  return result;
}
...
export async function loadPriceBook() {
  let records = await nexartClient.entities.PriceBookEntry.list('-created_date', 500);
  if (records.length === 0) {
    ...
    for (let i = 0; i < seedData.length; i += 50) {
      await nexartClient.entities.PriceBookEntry.bulkCreate(seedData.slice(i, i + 50));
    }
    records = await nexartClient.entities.PriceBookEntry.list('-created_date', 500);
  } else {
    // Merge any missing seed PB entries (e.g. Concrete)
    const services = await nexartClient.entities.Service.list('-created_date', 500);
...
export async function createPriceBookEntry(data, services = []) {
  ...
  const result = await nexartClient.entities.PriceBookEntry.create(resolved);
  await invalidateAllCaches();
  return result;
}
...
export async function updatePriceBookEntry(id, data, services = []) {
  const resolved = resolveServiceId(data, services);
  const result = await nexartClient.entities.PriceBookEntry.update(id, resolved);
  await invalidateAllCaches();
  return result;
}
...
export async function importPriceBookEntries(rows, existingEntries, services) {
  ...
  for (const row of rows) {
    ...
    if (existing) {
      ...
      await nexartClient.entities.PriceBookEntry.update(existing.id, {
      ...
      if (!svcMatch) {
        await nexartClient.entities.Service.create({
          name,
          category: row.category || 'Misc',
          unit: row.uom || 'each',
          type: row.type || 'service',
