...
export async function findTransmissionByMessageId(messageId) {
  try {
    const transmissions = await nexartClient.asServiceRole.entities.EstimateTransmission.filter(
...
export async function applyWebhookEvent(transmission, event) {
  ...
  try {
    await nexartClient.asServiceRole.entities.EstimateTransmission.update(
      transmission.id,
      updates
    );
