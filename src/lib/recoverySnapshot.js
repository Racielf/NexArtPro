...
export async function writeVaultSnapshot(entityName, record, deletedBy, reason = '') {
  try {
    ...
    await nexartClient.entities.RecoveryVault.create(payload);
  } catch (err) {
...
export async function markVaultRestored(entityId, restoredBy) {
  try {
    const vaultEntries = await nexartClient.entities.RecoveryVault.filter({ entity_id: entityId });
    ...
    if (!target) return;
    await nexartClient.entities.RecoveryVault.update(target.id, {
      is_restored: true,
