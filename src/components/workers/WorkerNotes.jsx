...
export default function WorkerNotes({ worker }) {
  ...
  const loadNotes = async () => {
    setLoading(true);
    const data = await base44.entities.WorkerNote.filter({ worker_id: worker.id }, '-created_date');
    setNotes(data);
    setLoading(false);
  };
  ...
  const handleAdd = async () => {
    ...
    await base44.entities.WorkerNote.create({
      worker_id: worker.id,
  ...
  const handleDelete = async (id) => {
    ...
    await base44.entities.WorkerNote.delete(id);
    loadNotes();
  };
