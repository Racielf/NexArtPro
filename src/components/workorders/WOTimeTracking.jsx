...
export default function WOTimeTracking({ workOrderId, workOrder, initialArrival, initialDeparture }) {
  ...
  const loadSessions = async () => {
    ...
    const entries = await nexartClient.entities.WorkOrderTimeEntry.filter({ work_order_id: workOrderId });
    setSessions(entries || []);
    setLoadingSessions(false);
  };

  const loadWorkers = async () => {
    const all = await nexartClient.entities.Worker.filter({ active: true });
    setWorkers(all || []);
  };
  ...
  const handleAddSession = async () => {
    ...
    await nexartClient.entities.WorkOrderTimeEntry.create(entry);
    ...
    if (todaySessions.length === 0 && worker.id === workOrder?.assigned_worker_id) {
      ...
      await nexartClient.entities.WorkOrder.update(workOrderId, legacyPatch);
      setArrival(newStart);
  ...
  const handleCloseSession = async (session) => {
    ...
    await nexartClient.entities.WorkOrderTimeEntry.update(session.id, {
      end_time: endTime,
      duration_hours: duration,
    });
    // Update legacy departure_time only for main assigned worker
    if (session.worker_id === workOrder?.assigned_worker_id) {
      await nexartClient.entities.WorkOrder.update(workOrderId, { departure_time: endTime });
