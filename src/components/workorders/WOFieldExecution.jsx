...
export default function WOFieldExecution({ workOrder, workOrderId, onUpdate }) {
  ...
  const addFieldNote = async () => {
    ...
    setNewNote('');
    await nexartClient.entities.WorkOrder.update(workOrderId, { field_notes: updated });
    setSavingNote(false);
  ...
  const persistChecklist = async (items) => {
    await nexartClient.entities.WorkOrder.update(workOrderId, { execution_checklist: items });
  };
  ...
  const handleDeletePhoto = async (photoId) => {
    ...
    await nexartClient.entities.ProjectPhoto.delete(photoId);
