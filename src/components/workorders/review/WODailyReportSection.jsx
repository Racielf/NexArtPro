...
export default function WODailyReportSection({ workOrder, woId }) {
  ...
  const load = async () => {
    const data = await nexartClient.entities.WorkOrderDailyReport.filter({ work_order_id: woId }, '-work_date');
    setReports(data);
    setLoading(false);
  };
  ...
  const handleSave = async () => {
    ...
    if (editing === 'new') {
      await nexartClient.entities.WorkOrderDailyReport.create(payload);
      toast.success('Daily report added');
    } else {
      await nexartClient.entities.WorkOrderDailyReport.update(editing, payload);
