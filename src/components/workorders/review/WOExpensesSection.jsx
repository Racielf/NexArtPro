...
export default function WOExpensesSection({ workOrder, woId }) {
  ...
  const load = async () => {
    const data = await nexartClient.entities.WorkOrderExpense.filter({ work_order_id: woId }, '-created_date');
    setExpenses(data);
  };
  ...
  const handleSave = async () => {
    ...
    if (editing === 'new') {
      await nexartClient.entities.WorkOrderExpense.create(payload);
      toast.success('Expense added');
    } else {
      await nexartClient.entities.WorkOrderExpense.update(editing, payload);
  ...
  const handleDelete = async (id) => {
    await nexartClient.entities.WorkOrderExpense.delete(id);
