...
export default function PhotoGallery({ workOrderId, appointmentId, customerId, customerName, workOrderNumber }) {
  ...
  const loadPhotos = async () => {
    ...
    const data = Object.keys(filter).length
      ? await nexartClient.entities.ProjectPhoto.filter(filter, '-created_date')
      : [];
    setPhotos(data);
    setLoading(false);
  };
  ...
  const handleUpload = async (phase, files) => {
    ...
    for (const file of Array.from(files)) {
      ...
      const { file_url } = await nexartClient.integrations.Core.UploadFile({ file: resized });
  ...
  const handleDelete = async (photo) => {
    await nexartClient.entities.ProjectPhoto.delete(photo.id);
