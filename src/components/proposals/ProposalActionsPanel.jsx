...
function ConvertToInvoiceBtn({ proposal, onConverted }) {
  ...
  const handle = async () => {
    ...
    const inv = await nexartClient.entities.Invoice.create({
    ...
    await nexartClient.entities.Proposal.update(proposal.id, {
      invoice_id: inv.id,
...
function ConvertToWorkOrderBtn({ proposal, onConverted }) {
  ...
  const handle = async () => {
    ...
    const wo = await nexartClient.entities.WorkOrder.create({
...
function ApplyAdjustmentBtn({ proposal, onConverted }) {
  ...
  const handle = async () => {
    ...
    const ests = await nexartClient.entities.Estimate.filter({ id: proposal.adjustment_estimate_id });
    const est = ests[0];
    if (!est) {
    ...
    await nexartClient.entities.Proposal.update(proposal.id, syncedFields);
    setLoading(false);
...
export default function ProposalActionsPanel({ proposal: proposalProp, onStatusChange, onOpenPreview, onOpenSend, pdfElementRef }) {
  ...
  const transition = async (newStatus, extra = {}) => {
    setLoading(true);
    await nexartClient.entities.Proposal.update(proposalId, { status: newStatus, ...extra });
  ...
  const handleCloseDeal = async (closingData) => {
    setLoading(true);
    await nexartClient.entities.Proposal.update(proposalId, closingData);
