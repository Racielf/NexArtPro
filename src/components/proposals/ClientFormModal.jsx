...
/**
 * ClientFormModal — create or edit a nexartClient Client record.
 * Props:
 *   open         — boolean
...
export default function ClientFormModal({ open, onOpenChange, client = null, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    try {
      let saved;
      if (client?.id) {
        saved = await nexartClient.entities.Client.update(client.id, data);
        toast.success('Customer updated');
      } else {
        saved = await nexartClient.entities.Client.create(data);
      ...
      if (err?.message?.includes('not found')) {
        ...
        try {
          const saved = await nexartClient.entities.Client.create(data);
          onOpenChange(false);
