...
export default function ApptDetailPanel({ appt, onClose, onEdit, onStatusChange, onRefresh }) {
  ...
  const loadRelated = async () => {
    ...
    let estResults = [];
    if (appt.estimate_id) {
      estResults = await nexartClient.entities.Estimate.filter({ id: appt.estimate_id }, '-created_date', 1);
    }
    if (estResults.length === 0 && appt.customer_id) {
      estResults = await nexartClient.entities.Estimate.filter({ client_id: appt.customer_id }, '-created_date', 1);
    }

    const [wos, invs] = await Promise.all([
      appt.customer_id
        ? nexartClient.entities.WorkOrder.filter({ client_id: appt.customer_id }, '-created_date', 1)
        : Promise.resolve([]),
      appt.customer_id
        ? nexartClient.entities.Invoice.filter({ client_id: appt.customer_id }, '-created_date', 1)
        : Promise.resolve([]),
    ]);
