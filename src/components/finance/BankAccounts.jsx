...
export default function BankAccounts() {
  ...
  const loadData = async () => {
    const [accs, txs, wos, custs] = await Promise.all([
      nexartClient.entities.BankAccount.list('-created_date'),
      nexartClient.entities.BankTransaction.list('-date', 200),
      nexartClient.entities.WorkOrder.list('-created_date', 100),
      nexartClient.entities.Customer.list('-created_date', 100),
    ]);
    setAccounts(accs);
    setTransactions(txs);
    setWorkOrders(wos);
  ...
  const saveAccount = async () => {
    ...
    await nexartClient.entities.BankAccount.create(data);
  ...
  const deleteAccount = async (id) => {
    ...
    await nexartClient.entities.BankAccount.delete(id);
  ...
  const saveTx = async () => {
    ...
    if (editingTx) {
      await nexartClient.entities.BankTransaction.update(editingTx.id, data);
      toast.success('Transaction updated');
    } else {
      await nexartClient.entities.BankTransaction.create(data);
  ...
  const deleteTx = async (id) => {
    ...
    await nexartClient.entities.BankTransaction.delete(id);
    toast.success('Transaction deleted');
    loadData();
  };

  const toggleReviewed = async (tx) => {
    await nexartClient.entities.BankTransaction.update(tx.id, { is_reviewed: !tx.is_reviewed });
    loadData();
  };
