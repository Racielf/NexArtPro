...
export default function ClientDocuments({ client }) {
  ...
  const load = async () => {
    ...
    const [ests, wos, invs] = await Promise.all([
      nexartClient.entities.Estimate.filter({ client_id: client.id }, '-created_date', 5),
      nexartClient.entities.WorkOrder.filter({ client_id: client.id }, '-created_date', 5),
      nexartClient.entities.Invoice.filter({ client_id: client.id }, '-created_date', 5),
    ]);
    setEstimates(ests);
    setWorkOrders(wos);
    setInvoices(invs);
