    ...
    <div className="w-full space-y-0 max-w-5xl mx-auto">
      ...
      <div className="bg-white rounded-xl border border-slate-100 overflow-hidden mb-5" style={{ boxShadow: '0 4px 14px rgba(15,23,42,0.05), 0 1px 3px rgba(15,23,42,0.04)' }}>
        <div className="flex items-center justify-between px-6 py-3 border-b border-slate-100 bg-slate-50/60">
          ...
          {!isPreview && (() => {
            const marginStatus = grossMarginPct < 25 ? { label: 'Critical', dot: 'bg-red-500', text: 'text-red-700', bg: 'bg-red-50 border-red-200' } : grossMarginPct <= 40 ? { label: 'Warning', dot: 'bg-amber-400', text: 'text-amber-700', bg: 'bg-amber-50 border-amber-200' } : { label: 'Healthy', dot: 'bg-emerald-500', text: 'text-emerald-700', bg: 'bg-emerald-50 border-emerald-200' };
        ...
        <div className="flex gap-0 flex-wrap">
          {!isPreview && (() => {
            ...
            return <div className="flex-1 min-w-[280px] px-6 py-5 border-r border-slate-100"><div className="flex items-center justify-between gap-3 mb-3"><p className="text-[9px] font-bold tracking-widest uppercase text-slate-700">🔒 Internal · Business Profit Analysis</p><span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[10px] font-bold ${marginStatus.bg} ${marginStatus.border} ${marginStatus.text}`}><span className="w-1.5 h-1.5 rounded-full bg-current" />{marginStatus.label} · {grossMarginPct.toFixed(1)}%</span></div><div className="grid grid-cols-2 gap-2"><div className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5"><p className="text-[9px] font-bold uppercase tracking-wide text-slate-700 mb-1">Revenue</p><p className="text-sm font-bold text-slate-800 tabular-nums">{fmt(total)}</p></div>{materialsSubtotal > 0 && <div className="bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2.5"><p className="text-[9px] font-bold uppercase tracking-wide text-emerald-700 mb-1">Materials cost</p><p className="text-sm font-bold text-emerald-700 tabular-nums">{fmt(materialsCost)}</p></div>}{otherCostsTotal > 0 && <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2.5"><p className="text-[9px] font-bold uppercase tracking-wide text-amber-700 mb-1">Other costs</p><p className="text-sm font-bold text-amber-700 tabular-nums">{fmt(otherCostsTotal)}</p></div>}<div className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5"><p className="text-[9px] font-bold uppercase tracking-wide text-slate-700 mb-1">Decision costs</p><p className="text-sm font-bold text-slate-700 tabular-nums">{fmt(totalCost)}</p></div><div className={`border rounded-lg px-3 py-2.5 ${marginStatus.bg} ${marginStatus.border}`}><p className="text-[9px] font-bold uppercase tracking-wide text-slate-700 mb-1">Profit</p><div className="flex items-baseline gap-2"><p className={`text-base font-bold tabular-nums ${marginStatus.text}`}>{fmt(netProfit)}</p></div></div><div className={`border rounded-lg px-3 py-2.5 ${marginStatus.bg} ${marginStatus.border}`}><p className="text-[9px] font-bold uppercase tracking-wide text-slate-700 mb-1">Margin %</p><div className="flex items-baseline gap-2"><p className={`text-base font-bold tabular-nums ${marginStatus.text}`}>{netProfitPct.toFixed(1)}%</p></div></div></div></div>;
          })()}
