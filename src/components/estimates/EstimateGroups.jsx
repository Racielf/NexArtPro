      ...
      <div className="bg-white rounded-xl border border-slate-100 overflow-hidden mb-5" style={{ boxShadow: '0 4px 14px rgba(15,23,42,0.05), 0 1px 3px rgba(15,23,42,0.04)' }}>
        ...
        <div className="flex gap-0 flex-wrap">
          {!isPreview && (() => {
            ...
            return <div className="flex-1 min-w-[280px] px-6 py-5 border-r border-slate-100"><div className="flex items-center justify-between gap-3 mb-3"><p className="text-[9px] font-bold tracking-widest uppercase text-slate-700">🔒 Internal · Análisis financiero / Financial Analysis</p><span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full border text-[10px] font-bold ${marginStatus.bg} ${marginStatus.border} ${marginStatus.text}`}><span className="w-1.5 h-1.5 rounded-full bg-current" />{marginStatus.label} · {grossMarginPct.toFixed(1)}%</span></div><div className="grid grid-cols-2 gap-2"><div className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5"><p className="text-[9px] font-bold uppercase tracking-wide text-slate-700 mb-1">Cost base / Base de costo</p><p className="text-sm font-bold text-slate-800 tabular-nums">{fmt(totalCost)}</p></div><div className="bg-slate-50 border border-slate-200 rounded-lg px-3 py-2.5"><p className="text-[9px] font-bold uppercase tracking-wide text-slate-700 mb-1">Revenue / Ingreso</p><p className="text-sm font-bold text-slate-800 tabular-nums">{fmt(total)}</p></div>{materialsSubtotal > 0 && <div className="bg-emerald-50 border border-emerald-200 rounded-lg px-3 py-2.5"><p className="text-[9px] font-bold uppercase tracking-wide text-emerald-700 mb-1">Costo de materiales</p><p className="text-sm font-bold text-emerald-700 tabular-nums">{fmt(materialsCost)}</p>{!billMaterialsToClient && <p className="text-[9px] font-semibold text-emerald-700 mt-1">Internal cost only</p>}</div>}{otherCostsTotal > 0 && <div className="bg-amber-50 border border-amber-200 rounded-lg px-3 py-2.5"><p className="text-[9px] font-bold uppercase tracking-wide text-amber-700 mb-1">Otros gastos</p><p className="text-sm font-bold text-amber-700 tabular-nums">{fmt(otherCostsTotal)}</p></div>}<div className={`border rounded-lg px-3 py-2.5 ${marginStatus.bg} ${marginStatus.border}`}><p className="text-[9px] font-bold uppercase tracking-wide text-slate-700 mb-1">Profit / Ganancia</p><div className="flex items-baseline gap-2"><p className={`text-base font-bold tabular-nums ${marginStatus.text}`}>{fmt(netProfit)}</p></div></div><div className="bg-blue-50 border border-blue-200 rounded-lg px-3 py-2.5"><p className="text-[9px] font-bold uppercase tracking-wide text-blue-700 mb-1">Markup % / Construcción del precio</p><p className="text-base font-bold text-blue-700 tabular-nums">{markupPercentage.toFixed(1)}%</p></div><div className={`border rounded-lg px-3 py-2.5 ${marginStatus.bg} ${marginStatus.border}`}><p className="text-[9px] font-bold uppercase tracking-wide text-slate-700 mb-1">Margin % / Desempeño del negocio</p><div className="flex items-baseline gap-2"><p className={`text-base font-bold tabular-nums ${marginStatus.text}`}>{netProfitPct.toFixed(1)}%</p></div></div></div></div>;
          })()}
