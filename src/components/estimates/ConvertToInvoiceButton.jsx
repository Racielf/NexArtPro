...
export default function ConvertToInvoiceButton({ estimate, onConverted, asDropdownItem = false }) {
  ...
  const handleConvert = async () => {
    ...
    setLoading(true);
    try {
      const existing = await nexartClient.entities.Invoice.filter({ estimate_id: estimate.id });
      ...
      if (!work_order_id) {
        const woList = await nexartClient.entities.WorkOrder.filter({ estimate_id: estimate.id });
        work_order_id = woList?.[0]?.id || null;
      }

      const invoice = await nexartClient.entities.Invoice.create({
