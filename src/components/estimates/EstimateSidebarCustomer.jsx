...
export default function EstimateSidebarCustomer({ estimate, client: clientProp, onCustomerChange, onAttachmentsUpdate }) {
  ...
  useEffect(() => {
    nexartClient.entities.Client.list('-created_date', 50)
  ...
  useEffect(() => {
    ...
    nexartClient.entities.Client.filter({ id: clientId }).then(res => {
      const found = res[0];
      ...
      nexartClient.entities.Customer.filter({ id: clientId }).then(custRes => {
