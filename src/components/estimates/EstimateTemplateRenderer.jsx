 ...
 * Genera Estimados, Work Orders e Invoices con estructura corporativa profesional
 * 
 * Props:
 * - estimate: datos del documento
 ...
 * - documentType: 'estimate' | 'workorder' | 'invoice'
 * 
 * LÓGICA ESPECIAL:
 * - Work Order: Oculta automáticamente precios, impuestos, descuentos, depósitos
 * - Invoice: Muestra precio, impuestos, descuentos (sin depósito)
 * - Estimate: Muestra todo (precios, depósito, términos)
 */
export default function EstimateTemplateRenderer({ estimate, template = 'professional', options = {}, documentType = 'estimate' }) {
  ...
  // LÓGICA: Work Order nunca muestra precios
  ...
  // ────────────────────────────────────────────────────────────────────────
  const CorporateHeader = ({ docType = 'ESTIMATE' }) => (
    <div style={{ background: '#0f172a', padding: '36px 52px 30px', color: 'white' }}>
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 20 }}>
        ...
        <div style={{ textAlign: 'right' }}>
          <div style={{ color: '#38bdf8', fontSize: 11, fontWeight: 600, letterSpacing: '0.12em', textTransform: 'uppercase', marginBottom: 6 }}>{docType}</div>
  ...
  // ────────────────────────────────────────────────────────────────────────
  if (template === 'professional' || template === undefined) {
    return (
      <div id="estimate-document" style={bodyStyle}>
        {/* CORPORATE HEADER */}
        <CorporateHeader docType={isWorkOrder ? 'WORK ORDER' : isInvoice ? 'INVOICE' : 'ESTIMATE'} />
  ...
  // Para otras plantillas (compact, modern, etc.) reutilizar template professional por ahora
  return null;
}