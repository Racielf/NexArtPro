...
export default function WOReceiptsSection({ workOrder, woId }) {
  ...
  const load = async () => {
    const [r, e] = await Promise.all([
      base44.entities.WorkOrderReceipt.filter({ work_order_id: woId }, '-created_date'),
      base44.entities.WorkOrderExpense.filter({ work_order_id: woId }),
    ]);
    setReceipts(r);
    setExpenses(e);
  };
  ...
  const handleDelete = async (id) => {
    await base44.entities.WorkOrderReceipt.delete(id);
