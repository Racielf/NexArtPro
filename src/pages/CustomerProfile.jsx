...
export default function CustomerProfile() {
  ...
  const loadAll = async () => {
    ...
    const [cust, appts, ests, wos, invs] = await Promise.all([
      nexartClient.entities.Customer.filter({ id: customerId }),
      nexartClient.entities.Appointment.filter({ customer_id: customerId }, '-created_date', 20),
      nexartClient.entities.Estimate.filter({ client_id: customerId }, '-created_date', 20),
      nexartClient.entities.WorkOrder.filter({ client_id: customerId }, '-created_date', 20),
      nexartClient.entities.Invoice.filter({ client_id: customerId }, '-created_date', 20),
    ]);
    if (cust.length) {
      setCustomer(cust[0]);
  ...
  const handleSaveNote = async () => {
    await nexartClient.entities.Customer.update(customerId, { internal_notes: noteText });
  ...
  const handleCreateEstimate = async () => {
    ...
    const est = await nexartClient.entities.Estimate.create({
