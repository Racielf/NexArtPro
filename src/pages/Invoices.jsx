...
export default function Invoices() {
  ...
  const loadData = async () => {
    setLoading(true);
    const data = await nexartClient.entities.Invoice.list('-created_date');
  ...
  const handleSend = async (inv) => {
    await nexartClient.entities.Invoice.update(inv.id, { status: 'sent', sent_at: new Date().toISOString() });
  ...
  const handleConfirmBulkArchive = async (reason) => {
    ...
    await archiveManyWithSnapshot(nexartClient.entities.Invoice, 'Invoice', idsArray, actor, reason);
    setSelectedIds(new Set());
  ...
  const handleOneClickFollowUp = async (e, inv) => {
    ...
    await executeOneClickFollowUp(inv, nexartClient);
                         ...
                         <div onClick={e => e.stopPropagation()} className="flex-shrink-0">
                           <BillingIssueOwnerSelect
                             ...
                             onAssign={async (owner) => {
                               ...
                               if (normalized) {
                                 await nexartClient.entities.Invoice.update(inv.id, { billing_issue_owner: normalized });
                                 loadData();
                               }
                             }}
                           />
