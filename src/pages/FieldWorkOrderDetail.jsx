...
export default function FieldWorkOrderDetail() {
  ...
  const load = async () => {
    ...
    const [wo, ph, invList] = await Promise.all([
      nexartClient.entities.WorkOrder.get(id),
      nexartClient.entities.ProjectPhoto.filter({ work_order_id: id }),
      nexartClient.entities.Invoice.filter({ work_order_id: id }),
    ]);
    setWorkOrder(wo);
    setPhotos(ph || []);
  ...
  const handleCheckIn = async () => {
    ...
    try {
      await nexartClient.entities.WorkOrder.update(workOrder.id, {
