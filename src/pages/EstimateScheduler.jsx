...
export default function EstimateScheduler() {
  ...
  const loadData = async () => {
    const [appts, cls] = await Promise.all([
      nexartClient.entities.Appointment.list('-created_date'),
      nexartClient.entities.Client.list('-created_date'),
    ]);
    ...
    if (estimateId) {
      const ests = await nexartClient.entities.Estimate.filter({ id: estimateId });
      if (ests.length) setEstimate(ests[0]);
    } else {
      // load most recent estimate
      const ests = await nexartClient.entities.Estimate.list('-created_date', 1);
      if (ests.length) setEstimate(ests[0]);
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const newAppt = await nexartClient.entities.Appointment.create({
