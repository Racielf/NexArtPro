...
export default function Payments({ fieldMode = false }) {
  ...
  const loadData = async () => {
    setLoading(true);
    const invs = await base44.entities.Invoice.list('-created_date', 200).catch(() => []);
    setInvoices(invs || []);
    setLoading(false);
  };
  ...
  const handleRecordPayment = async () => {
    ...
    try {
      ...
      await sendPaymentReceipt({ base44, invoice: updatedInvoice, payment, balanceDue: state.balance_due });
