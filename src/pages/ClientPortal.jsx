...
export default function ClientPortal() {
  ...
  const handleLogin = async ({ email, phone }) => {
    ...
    const customers = await nexartClient.entities.Customer.filter({ email });
    const normalizedInput = normalizePhone(phone);
    const match = customers.find(c => normalizePhone(c.phone) === normalizedInput);

    if (!match) {
      const clients = await nexartClient.entities.Client.filter({ email });
  ...
  const loadDocuments = async () => {
    ...
    const [est, wo, inv] = await Promise.all([
      nexartClient.entities.Estimate.filter({ client_name: clientName }, '-created_date', 50).catch(() => []),
      nexartClient.entities.WorkOrder.filter({ client_name: clientName }, '-created_date', 50).catch(() => []),
      nexartClient.entities.Invoice.filter({ client_name: clientName }, '-created_date', 50).catch(() => []),
    ]);
