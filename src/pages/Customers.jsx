...
export default function Customers() {
  ...
  const loadCustomers = async () => {
    ...
    const [data, invData] = await Promise.all([
      ...
      nexartClient.entities.Invoice.list('-created_date', 500),
    ]);
  ...
  const handleConfirmArchive = async (reason) => {
    ...
    await archiveWithSnapshot(nexartClient.entities.Customer, 'Customer', id, actor, reason);
  ...
  const handleConfirmBulkArchive = async (reason) => {
    ...
    await archiveManyWithSnapshot(nexartClient.entities.Customer, 'Customer', idsArray, actor, reason);
    setSelectedIds(new Set());
