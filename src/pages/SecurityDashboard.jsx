...
import {
  Shield, AlertTriangle, Activity, Lock, Search,
  Clock, CheckCircle2, XCircle, RefreshCw, ChevronDown, ChevronUp,
  Trash2, RotateCcw, Eye, User, FileText
} from 'lucide-react';
...
const SEVERITY_STYLE = {
  INFO:     { badge: 'bg-blue-50 text-blue-700 border border-blue-200',     dot: 'bg-blue-400',   row: '' },
  WARNING:  { badge: 'bg-amber-50 text-amber-700 border border-amber-200',  dot: 'bg-amber-400',  row: 'bg-amber-50/30' },
  CRITICAL: { badge: 'bg-red-50 text-red-700 border border-red-200',        dot: 'bg-red-500',    row: 'bg-red-50/30' },
};
...
const AUDIT_ACTION_STYLE = {
  archive: { label: 'Archive',  badge: 'bg-amber-50 text-amber-700 border border-amber-200',   icon: FileText },
  restore: { label: 'Restore',  badge: 'bg-blue-50 text-blue-700 border border-blue-200',       icon: RotateCcw },
  purge:   { label: 'Purge',    badge: 'bg-red-50 text-red-700 border border-red-200',          icon: Trash2 },
};
      ...
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-slate-100">
          {events.map(ev => {
            ...
            const sStyle = SEVERITY_STYLE[severity];
            const isExp = expanded === ev.id;
            return (
              <React.Fragment key={ev.id}>
                <tr className={`hover:bg-slate-50 transition-colors ${sStyle.row}`}>
                  ...
                  <td className="px-4 py-3">
                    ...
                    {ev.origin_path && (
                      <p className="text-[10px] text-slate-400 mt-0.5 font-mono">{ev.origin_path}</p>
                    )}
                  </td>
                  <td className="px-4 py-3"><SeverityBadge severity={severity} /></td>
                  <td className="px-4 py-3">
                    {ev.success
                      ? <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full"><CheckCircle2 className="w-3 h-3" />Success</span>
                      : <span className="inline-flex items-center gap-1 text-[10px] font-semibold text-red-700 bg-red-50 border border-red-200 px-2 py-0.5 rounded-full"><XCircle className="w-3 h-3" />Failed</span>
                    }
                  </td>
                  <td className="px-4 py-3">
                    <button
                  ...
                  <tr className="bg-slate-50 border-b border-slate-200">
                    <td colSpan={6} className="px-6 py-4">
                      <div className="space-y-2 text-xs">
                        {ev.reason && (
                          <div><span className="font-semibold text-slate-600">Reason: </span><span className="text-slate-700">{ev.reason}</span></div>
                        )}
                        {ev.metadata_json && (
                          <div>
  ...
  return (
    <div className="overflow-x-auto">
      <table className="w-full text-sm">
        ...
        <tbody className="divide-y divide-slate-100">
          {entries.map(entry => {
            const aStyle = AUDIT_ACTION_STYLE[entry.action] || { label: entry.action, badge: 'bg-slate-100 text-slate-600 border border-slate-200', icon: Activity };
            ...
            return (
              <tr key={entry.id} className="hover:bg-slate-50 transition-colors">
                ...
                <td className="px-4 py-3">
                  <span className="flex items-center gap-1.5">
                    <User className="w-3 h-3 text-slate-400 flex-shrink-0" />
                    <span className="text-xs font-medium text-slate-700 truncate max-w-[140px]">{entry.performed_by || '—'}</span>
                  </span>
                </td>
                <td className="px-4 py-3">
                  <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full ${aStyle.badge}`}>
                ...
                <td className="px-4 py-3">
                  ...
                  {entry.entity_id && <p className="text-[10px] font-mono text-slate-400 mt-0.5 truncate max-w-[100px]">{entry.entity_id}</p>}
                </td>
...
export default function SecurityDashboard() {
  ...
  const filteredSec = useMemo(() => {
    ...
    // Date window
    if (dateFilter !== 'all') {
      const windows = { today: 1, '7days': 7, '30days': 30 };
      const days = windows[dateFilter] || 7;
      const cutoff = new Date(now - days * 24 * 60 * 60 * 1000);
      r = r.filter(e => new Date(e.created_date) >= cutoff);
    }
    if (severityFilter !== 'all') {
      r = r.filter(e => getSeverity(e.event_type) === severityFilter);
    }
    if (successFilter !== 'all') {
      r = r.filter(e => e.success === (successFilter === 'success'));
    }
    if (eventTypeFilter !== 'all') {
      r = r.filter(e => e.event_type === eventTypeFilter);
    }
    if (search.trim()) {
  ...
  const filteredAudit = useMemo(() => {
    ...
    if (dateFilter !== 'all') {
      ...
      const days = windows[dateFilter] || 7;
      const cutoff = new Date(now - days * 24 * 60 * 60 * 1000);
      r = r.filter(e => new Date(e.performed_at) >= cutoff);
    }
    if (search.trim()) {
      const q = search.toLowerCase();
      r = r.filter(e =>
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="bg-white border-b border-slate-200 px-6 py-5 flex items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Read-only · AuthSecurityLog + AuditLog</p>
          </div>
        </div>
      ...
      <div className="px-6 py-6 max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-5 gap-4">
          <KpiCard icon={Activity}      label="Total Events"     value={total}        sub="all time"       iconCls="bg-slate-100 text-slate-600"  borderCls="border-slate-200" />
          ...
          <KpiCard icon={RotateCcw}     label="Restores"         value={restoreCount} sub="all time"       iconCls="bg-blue-50 text-blue-600"     borderCls="border-slate-200" />
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm px-5 py-4">
          <div className="flex flex-wrap items-center gap-3">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <Input
                placeholder="Search user, event, reason…"
                value={search}
