...
function unwrapFnResult(res) {
  ...
  // nexartClient functionsProxy also wraps Ã¢â‚¬â€ handle both shapes
  return res?.data ?? res;
}
...
export default function SignDocumentView() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        const res = await nexartClient.functions.invoke('resolveSigningPackageToken', { token, fingerprint });
        if (cancelled) return;
  ...
  const requestOtpCode = async () => {
    ...
    try {
      ...
      const res = await nexartClient.functions.invoke('requestSigningOtp', { token, fingerprint });
  ...
  const verifyOtpCode = async () => {
    ...
    try {
      const fingerprint = deviceFingerprint || await getDeviceFingerprint();
      const res = await nexartClient.functions.invoke('verifySigningOtp', {
        token,
  ...
  const deliverSignedCopy = async () => {
    ...
    try {
      const res = await nexartClient.functions.invoke('sendSignedCopy', { token });
  ...
  const handleApprove = async () => {
    ...
    setActing(true);
    try {
      const res = await nexartClient.functions.invoke('completeSigningPackage', {
        token,
        action: 'approve',
  ...
  const handleDecline = async () => {
    ...
    setActing(true);
    try {
      await nexartClient.functions.invoke('completeSigningPackage', {
        token,
        action: 'decline',
