...
export default function InvoiceDetailClean() {
  ...
  const loadInvoice = async () => {
    ...
    if (inv) {
      ...
      if (inv.client_id) {
        try {
          const clientInvoices = await nexartClient.entities.Invoice.filter({ client_id: inv.client_id });
      ...
      if (inv.work_order_id) {
        try {
          ...
          const wo = woList?.[0];
          if (wo) {
            const photos = await nexartClient.entities.ProjectPhoto.filter({ work_order_id: inv.work_order_id });
