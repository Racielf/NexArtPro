...
export default function TimeTracking() {
  ...
  const loadEntries = async () => {
    setLoading(true);
    const data = await nexartClient.entities.TimeEntry.filter({ date: dateStr });
    setEntries(data);
  ...
  const loadAllEntries = async () => {
    ...
    const data = await nexartClient.entities.TimeEntry.list('-created_date', 200);
    setAllEntries(data);
  ...
  const startGPS = useCallback((entryId) => {
    ...
    navigator.geolocation.getCurrentPosition((pos) => {
      ...
      nexartClient.entities.TimeEntry.update(entryId, { start_lat: pos.coords.latitude, start_lng: pos.coords.longitude });
  ...
  const stopGPS = useCallback(async (entryId) => {
    ...
    navigator.geolocation.getCurrentPosition(async (pos) => {
      await nexartClient.entities.TimeEntry.update(entryId, { end_lat: pos.coords.latitude, end_lng: pos.coords.longitude, miles_traveled: parseFloat(currentMiles.toFixed(2)) });
  ...
  const handleStartTimer = async () => {
    ...
    const entry = await nexartClient.entities.TimeEntry.create({
  ...
  const handleStopTimer = async () => {
    ...
    await nexartClient.entities.TimeEntry.update(runningEntry.id, {
  ...
  const handleDelete = async (id) => {
    ...
    await nexartClient.entities.TimeEntry.delete(id);
