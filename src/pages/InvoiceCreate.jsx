...
import { ArrowLeft, CheckCircle2, FilePlus2, Plus, Receipt, Search, Trash2, UserPlus } from 'lucide-react';
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-[1fr_340px] gap-6">
          <div className="space-y-5">
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
              <div className="flex items-center justify-between gap-3 mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <h2 className="font-black text-slate-900">Client</h2>
              ...
              {clientMode === 'existing' ? (
                <div className="space-y-3">
                  <div className="relative">
                    ...
                    <Input
                      ...
                      placeholder="Search existing clients by name, phone, email, or address..."
                      className="pl-9"
                    />
                  </div>
                        ...
                        return (
                          ...
                          >
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${active ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500'}`}>
                              {active ? <CheckCircle2 className="w-4 h-4" /> : <span className="text-xs font-black">{client.full_name?.charAt(0)?.toUpperCase() || '?'}</span>}
                            </div>
                ...
                <div className="rounded-xl border border-blue-100 bg-blue-50/40 p-4 space-y-4">
                  <div className="flex items-center gap-2 text-blue-700">
                    ...
                    <p className="text-sm font-black">Create client inline</p>
                  </div>
            ...
            <div className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <Field label="Selected Client"><Input value={form.client_name} onChange={e => updateForm('client_name', e.target.value)} placeholder="Select or create a client above" disabled={!!selectedClient} /></Field>
          ...
          <div className="lg:sticky lg:top-6 h-fit bg-slate-950 text-white rounded-3xl shadow-2xl p-6 space-y-5">
            <div>
              ...
              <h3 className="text-2xl font-black mt-1">{form.client_name || 'Select Client'}</h3>
              <p className="text-sm text-slate-400 mt-1">{form.title || 'Manual invoice'}</p>
              {form.client_address && <p className="text-xs text-slate-500 mt-2 line-clamp-2">{form.client_address}</p>}
            </div>
