...
export default function ProposalEditor() {
  ...
  const { data: closedProposals = [] } = useQuery({
    ...
    queryFn: async () => {
      const list = await nexartClient.entities.Proposal.filter({ close_outcome: { $exists: true } });
      return list;
    },
  ...
  const { data: clientProposals = [] } = useQuery({
    ...
    queryFn: async () => {
      ...
      return await nexartClient.entities.Proposal.filter({ client_id: proposal.client_id }, '-created_date', 100);
    },
  ...
  const load = async () => {
    ...
    const list = await nexartClient.entities.Proposal.filter({ id: proposalId });
  ...
  const handleSave = async (estimateData) => {
    ...
    try {
      await nexartClient.entities.Proposal.update(proposalId, sanitized);
  ...
  const handleCustomerChange = async (customerData, clientRecord) => {
    ...
    await nexartClient.entities.Proposal.update(proposalId, updated);
    setProposal(updated);
    setSaving(false);
  ...
  const handleDiscard = async () => {
    if (proposalId) await nexartClient.entities.Proposal.delete(proposalId);
    navigate('/proposals');
  };
