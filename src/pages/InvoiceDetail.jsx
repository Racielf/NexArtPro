...
export default function InvoiceDetail() {
  ...
  const loadInvoice = async () => {
    ...
    const list = await nexartClient.entities.Invoice.filter({ id: invoiceId });
    if (list.length) {
      const inv = list[0];
      setInvoice(inv);
      setNotes(inv.notes || '');
      ...
      const allInvoices = await nexartClient.entities.Invoice.list('-created_date');
      setRecentOwners(getRecentOwners(allInvoices, 5));
      if (inv.work_order_id) {
        try {
          const woList = await nexartClient.entities.WorkOrder.filter({ id: inv.work_order_id });
          if (woList.length) {
            const wo = woList[0];
            setWorkOrder(wo);
  ...
  const handleSaveNotes = async () => {
    setSaving(true);
    await nexartClient.entities.Invoice.update(invoiceId, { notes, due_date: dueDate });
  ...
  const handleSend = async () => {
    ...
    await nexartClient.entities.Invoice.update(invoiceId, { status: 'sent', sent_at: now, last_contacted_at: now });
  ...
  const handleMarkPaid = async () => {
    ...
    await nexartClient.entities.Invoice.update(invoiceId, {
  ...
  const handleResolveBreach = async (breachType) => {
    ...
    await nexartClient.entities.Invoice.update(invoiceId, patch);
            ...
            <div className={`px-5 py-5 border-b border-slate-100 ${isPaid ? 'bg-emerald-50' : isOverdue ? 'bg-red-50' : 'bg-slate-50'}`}>
              ...
              {invoice.status === 'sent' && !isPaid && (
                <button
                  ...
                  onClick={async () => {
                    await markInvoiceContacted(invoiceId, nexartClient);
                  ...
                  {invoice.billing_issue_status === 'open' && (
                    <BillingIssueOwnerSelect
                      ...
                      onAssign={async (owner) => {
                        ...
                        if (normalized) {
                          setSaving(true);
                          await nexartClient.entities.Invoice.update(invoiceId, { billing_issue_owner: normalized });
