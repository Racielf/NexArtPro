...
export default function NexArtSign() {
  ...
  const load = async () => {
    ...
    try {
      const [pkgRows, eventRows, certRows, participantRows, estimateRows] = await Promise.all([
        nexartClient.entities.SigningPackage.list('-created_date').catch(() => []),
        nexartClient.entities.SigningEvent.list('-created_at').catch(() => []),
        nexartClient.entities.SigningCertificate.list('-generated_at').catch(() => []),
        nexartClient.entities.SigningParticipant.list('-created_date').catch(() => []),
        nexartClient.entities.Estimate.list('-created_date').catch(() => []),
      ]);
  ...
  const issueSigningUrl = async (pkg) => {
    ...
    const response = await nexartClient.functions.invoke('issueSigningAccessLink', {
  ...
  const writeEvent = async ({ pkg, event_type, metadata = {} }) => {
    if (!pkg?.id) return;
    const currentUser = await nexartClient.auth.me().catch(() => null);
  ...
  const closePackage = async (pkg, status) => {
    ...
    try {
      ...
      await Promise.all(selectedParticipants
        ...
        .map(participant => nexartClient.entities.SigningParticipant.update(participant.id, {
          status,
          token: '',
          token_hash: '',
          token_last_four: '',
