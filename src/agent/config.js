...
export const DIFF_RULES = [
  ...
  // ── WARN RULES ──────────────────────────────────────────────────────────────
  {
    ...
    severity: 'high',
    suggestion: "Replace nexartClient.entities.Client with nexartClient.entities.Customer and update any client_id references to customer_id. Use the dual-lookup pattern in EstimateSidebarCustomer only where backward compat is explicitly needed.",
    validate({ filePath, diff }) {
      ...
      return {
        valid: false,
        type: 'warn',
        message: `Estimate module "${filePath}" is calling nexartClient.entities.Client directly.`,
      };
    },
  },
  {
