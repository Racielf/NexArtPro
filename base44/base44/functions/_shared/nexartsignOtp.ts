...
export async function persistOtpState(base44: any, context: any, otpState: Record<string, unknown> | null) {
  if (!context?.pkg) return;

  if (context.hasParticipants && context.matchedParticipant?.id) {
    await base44.asServiceRole.entities.SigningParticipant.update(context.matchedParticipant.id, {
      metadata: {
        ...(context.matchedParticipant.metadata || {}),
  ...
  await base44.asServiceRole.entities.SigningPackage.update(context.pkg.id, {
    audit_summary: {
      ...(context.pkg.audit_summary || {}),
      nexartsign_otp: otpState,
    },
  }).catch(() => {});
}
