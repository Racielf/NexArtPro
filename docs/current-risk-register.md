...
| `document_type` coercion in `EstimateEditor.loadEstimate` (PROPOSAL → ESTIMATE) | Pending verification — may cause silent coercion of `PROPOSAL` documents to `ESTIMATE` type on load; requires a targeted test before fix |
| `origin_type` on WorkOrder created from Proposal | Pending — value may be set to `"estimate"` instead of `"proposal"`; low urgency but worth correcting for traceability |
